...
document.body.addEventListener("click", () => {
    ...
    // 배경을 그라디언트만 남기도록 설정
    introContainer.style.background = "linear-gradient(to bottom, black 65%, #d6d6d6,  #FFD8C7)";
    if (messageIndex < messages.length) {
        introContainer.querySelector('h1').style.display = "none";  // 썸네일 숨김
        ...
        englishText.style.fontSize = "1.5em"; // 영어 메시지 스타일
        englishText.style.opacity = 0; // 초기 불투명도
        englishText.style.transition = "opacity 1s ease-out"; // 애니메이션 효과
        englishText.style.color = "#4b4b4b";
        englishText.style.zIndex = -1;
        ...
        // 약간의 지연 후 애니메이션 시작
        setTimeout(() => {
            introText.style.transition = "opacity 1s ease-out, transform 1s ease-out"; // 다시 transition 설정
